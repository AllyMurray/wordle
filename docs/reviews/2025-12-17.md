...
1. **Unused types** - `UseWordleOptions`, `UseWordleReturn`, `UseMultiplayerReturn` in `types.ts` appear to be legacy from previous hook-based implementation.

2. **ESLint disable comment** at `src/hooks/useGameSession.ts:180` for exhaustive deps - could be cleaner with a custom hook.
...
| Low | Remove unused types | Clean up legacy type definitions |
